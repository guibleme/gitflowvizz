...
export class GanttComponent implements OnInit, AfterViewInit, OnChanges, AfterContentChecked {
  ...
  @Input()
  public colorByStatus: boolean = false;
  ...
  constructor() {
    this.taskDataManipulator = new TaskDataManipulator(this.colours, this.enableGroup, true)
    ...
    return {
      ...
      formatter: function (info:any) {
          ...
          return [
              ...
              + `<br> Owner: ${info.value[6]}`
              + `<br> alive for ${info.value[11]} minutes`
              + `<br> ${info.value[12]} on ${info.value[13]}`
