...
export class TaskDataManipulator {
    ...
    constructor(colours:string[], enableGroup:boolean = true, colorByStatus: boolean){
      this.COLOURS = colours
      this._enableGroup = enableGroup
      this.colorByStatus = colorByStatus;
    }
    ...
    mapData(taskData:TaskModel[]): any[]{
        ...
        for(let index = 0; index < taskData.length; index++){
            ...
            } else if (this.colorByStatus) {
              if (taskData[index].status) {
                color = taskData[index].status === 'failed' ? '#c15046': '#5fb07d'
              } else {
            ...
            let index_attributes = [index, item.taskName, item.start, item.end, item.taskId, item.donePercentage, item.owner, item.image, item.groupName, isToDrawGroup, color, item.time, item.status, item.event];
